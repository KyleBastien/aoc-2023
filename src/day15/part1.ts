 ...
 */
export function runHashAlgorithm(input: string): number {
  let currentValue = 0;
...
function part1(initializationSequence: string): number {
  const sequences = initializationSequence.split(',');
  let sum = 0;
  for (const step of sequences) {
    const cleanedStep = step.replace(/\s/g, '');
    const stepResult = runHashAlgorithm(cleanedStep);
    sum += stepResult;
  }
  return sum;
}
