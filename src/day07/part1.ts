...
export type HandData = { sort: number[]; bid: number };

export function cardSort(a: number[], b: number[]): number {
...
export function frequency(arr: number[]): { [key: number]: number } {
  return arr.reduce((freq: { [key: number]: number }, val: number) => {
...
export function reduceSum(acc: number, val: number): number {
  return acc + val;
}

const STRENGTH = '23456789TJQKA';
function part1(data: Hand[]): number {
  return data
    .map((line: string) => {
