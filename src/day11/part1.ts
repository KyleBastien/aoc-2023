 ...
 */
// Function to find shortest path between galaxies
export function shortestPathOnGrid(start: number[], end: number[]): number {
  const [x1, y1] = start;
  const [x2, y2] = end;
...
function part1(input: string[]): number {
  ...
  const newGrid: string[][] = new Array(newRows.length).fill(0).map(() => []);
